...
export class GeminiService {
  ...
  private static getNetlifyFunctionUrl(): string {
    ...
    return isDev ? 'http://localhost:8888/.netlify/functions' : '/.netlify/functions';
  }
  ...
  private static async callGeminiDirectly(imageBase64: string, prompt: string): Promise<any> {
    const response = await fetch(`${config.GEMINI_API_URL}?key=${config.GEMINI_API_KEY_LOCAL}`, {
      method: 'POST',
      headers: {
  ...
  static async analyzeIngredients(imageBase64: string, productName?: string): Promise<string[]> {
    ...
    if (this.shouldUseLocalAPI()) {
      ...
      try {
        const prompt = `Please extract and list all ingredients from this ${productName || 'product'} label image. Return only the ingredients as a comma-separated list, nothing else.`;
        const analysisText = await this.callGeminiDirectly(imageBase64, prompt);
        return analysisText.split(',').map((ingredient: string) => ingredient.trim()).filter((ingredient: string) => ingredient.length > 0);
      } catch (error) {
    ...
    try {
      const response = await fetch(`${this.getNetlifyFunctionUrl()}/analyze-ingredients`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.analysis) {
        ...
        const ingredientsMatch = data.analysis.match(/\*\*Ingredient List\*\*:?\s*([^\n]*(?:\n(?!\*\*)[^\n]*)*)/i);
        if (ingredientsMatch) {
          const ingredientsText = ingredientsMatch[1];
          return ingredientsText
            .split(/[,\n]/)
            .map((ingredient: string) => ingredient.replace(/^\d+\.\s*|-\s*|\*\s*/g, '').trim())
...
Please provide your response in a structured format with clear sections.`;
        ...
        const analysisText = await this.callGeminiDirectly(product.imageUri, prompt);
  ...
  private static parseDetailedAnalysis(analysisText: string): ProductAnalysis {
    ...
    const healthMatch = analysisText.match(/health.*?(\d+)(?:\/10|\s*out\s*of\s*10)/i);
    const healthScore = healthMatch ? parseInt(healthMatch[1]) * 10 : Math.floor(Math.random() * 60) + 20;
    ...
    const concernsMatch = analysisText.match(/\*\*Concerns\*\*:?\s*([^\n]*(?:\n(?!\*\*)[^\n]*)*)/i);
    const benefitsMatch = analysisText.match(/\*\*Benefits\*\*:?\s*([^\n]*(?:\n(?!\*\*)[^\n]*)*)/i);
